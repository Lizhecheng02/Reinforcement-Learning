...
class Agent(object):
    ...
    def build_policy_network(self):
        ...
        policy = Model(input=[input_, advantages], output=[probs])
        policy.compile(optimizer=Adam(lr=self.lr), loss=custom_loss)
        predict = Model(input=[input_], output=[probs])
    ...
    def learn(self):
        ...
        actions = np.zeros([len(self.state_memory), self.n_actions])
        actions[np.arange(len(self.state_memory)), action_memory] = 1
