...
class ActorNetwork(nn.Module):
    def __init__(self, n_actions, input_dims, alpha,
                 fc1_dims=256, fc2_dims=256, ckpt_dir="./model"):
        super(ActorNetwork, self).__init__()

        self.checkpoint_file = os.path.join(ckpt_dir, "actor_ppo")
...
class CriticNetwork(nn.Module):
    def __init__(self, input_dims, alpha, fc1_dims=256, fc2_dims=256, ckpt_dir="./model"):
        super(CriticNetwork, self).__init__()

        self.checkpoint_file = os.path.join(ckpt_dir, "critic_ppo")
    ...
    def learn(self):
        for _ in range(self.n_epochs):
            ...
            for batch in batches:
                states = torch.tensor(
                    state_arr[batch], dtype=torch.float).to(self.actor.device)
                old_probs = torch.tensor(
                    old_prob_arr[batch]).to(self.actor.device)
                ...
                weighted_clipped_probs = torch.clamp(
                    prob_ratio, 1 - self.clip_ratio, 1 + self.clip_ratio) * advantage[batch]
